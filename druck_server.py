...
# Historical tracking (in-memory for now)
REGIME_HISTORY = []
PORTFOLIO_HISTORY = []

def add_regime_snapshot(score, vix, qqq_spx, correlation):
    """Store regime snapshot"""
    global REGIME_HISTORY
    snapshot = {
    ...
    REGIME_HISTORY.append(snapshot)
    # Keep last 30 days (assume 1 call per day)
    if len(REGIME_HISTORY) > 30:
        REGIME_HISTORY.pop(0)
    return snapshot

def add_portfolio_snapshot(portfolio_value, total_return):
    ...
    if len(PORTFOLIO_HISTORY) > 90:  # 90 days
        PORTFOLIO_HISTORY.pop(0)
    return snapshot
...
def calculate_position_pnl(positions):
    ...
    pnl_data.sort(key=lambda x: abs(x["pnl_pct"]), reverse=True)
    return pnl_data

def check_alerts(regime_score, vix, qqq_spx, positions):
    ...
    if vix > 35:
        alerts.append({
            "type": "critical",
            "title": "VIX SPIKE",
            "message": f"VIX at {vix:.1f} - Execute T4 hedges",
            "action": "Increase GLD/TLT allocation"
        })
    elif vix > 30:
        alerts.append({
            "type": "warning",
            "title": "VIX Elevated",
            "message": f"VIX at {vix:.1f} - Caution mode",
            "action": "Monitor for 35 trigger"
        })
    ...
    if qqq_spx > 0.092:
        alerts.append({
            "type": "opportunity",
            "title": "TECH BREAKOUT",
            "message": f"QQQ/SPX {qqq_spx:.3f} - Strong tech",
            "action": "Consider adding AVGO/SNDK"
        })
    ...
    for ticker, data in positions.items():
        ...
        if entry > 0:
            ...
            if loss_pct < -15:  # 15% stop-loss
                alerts.append({
                    "type": "critical",
                    "title": f"{ticker} STOP-LOSS",
                    "message": f"Down {loss_pct:.1f}%",
                    "action": f"Consider exiting {ticker}"
                })
...
@app.route('/history')
def get_history():
    """Return regime and portfolio history"""
    try:
        return jsonify({
            "success": True,
            "regime_history": REGIME_HISTORY[-30:],  # Last 30 days
            "portfolio_history": PORTFOLIO_HISTORY[-90:]  # Last 90 days
        })
...
@app.route('/pnl')
def get_pnl():
    """Return position-level P&L"""
    try:
        # Get current data first
        current_data = get_data_internal()
        if not current_data.get("success"):
            return jsonify({"success": False, "error": "Failed to get data"}), 500
        
        positions = current_data["data"]["positions"]
        pnl_data = calculate_position_pnl(positions)
        
        return jsonify({
            "success": True,
            "pnl": pnl_data,
            "total_pnl": sum([p["pnl_dollar"] for p in pnl_data]),
            "total_pnl_pct": current_data["data"]["portfolio"]["total_return"]
        })
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def get_alerts():
    ...
    try:
        ...
        regime = current_data["data"]["regime"]
        positions = current_data["data"]["positions"]
        
        alerts = check_alerts(
            regime["score"],
            regime["breakdown"]["vix"],
            regime["breakdown"]["qqq_spx"],
            positions
        )
...
def get_data_internal():
    """Internal version of get_data for reuse"""
    global data_cache
    current_time = time.time()
    
    if data_cache["data"] and (current_time - data_cache["last_update"]) < 300:
        return data_cache["data"]
    
    # Calculate regime and portfolio (existing logic)
    # This would be refactored from existing /data endpoint
